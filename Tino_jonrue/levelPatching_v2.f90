...
do rP=1,dimR
  	do tP=1,dimT-(jump)
		dnsTemp(rP,tP)=dns0(rP,tP)+dns0(rP,tP+jump)
    	end do
end do
...
do rP=1,dimR
  	do tP=dimT-(jump)+1,dimT
		dnsTemp(rP,tP)=dns0(rP,tP)+dns0(rP,tP+jump-dimT)
    	end do
end do

do tP=1,dimT
	do rP=1,dimR-(jump)
        dns1(rP,tP) = dnsTemp(rP,tP)+dnsTemp(rP+jump,tP)
    end do
end do
...
open (unit=4, file="output/force_t_levelPatching.txt")
open (unit=5, file="output/force_r_levelPatching.txt")
do t=1,dimT
    do r=1,dimR
        write(4,*)forceT(r,t)
        write(5,*)forceR(r,t)
