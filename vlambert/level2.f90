program Gravity
  ...
  open(unit=100, file="forces_level2.txt", action="write", status="replace")
  ...
  do tha = 1, Nth
     ...
     do thb = 1, Nth/4
        diff_theta = theta_i - th(4*thb)-offset_th
  ...
  do radius = 1, Nr/4
     r_prime = r(4*radius)-offset_r
     do angle = 1, Nth/4
        Mass(radius,angle) = -dA*(dens(4*radius-3,4*angle-3) &
             + dens(4*radius-3,4*angle-2) &
             + dens(4*radius-3,4*angle-1) &
             + dens(4*radius-3,4*angle) & 
             + dens(4*radius-2,4*angle-3) &
             + dens(4*radius-2,4*angle-2) &
             + dens(4*radius-2,4*angle-1) &
             + dens(4*radius-2,4*angle) &
             + dens(4*radius-1,4*angle-3) &
             + dens(4*radius-1,4*angle-2) &
             + dens(4*radius-1,4*angle-1) &
             + dens(4*radius-1,4*angle) &
             + dens(4*radius,4*angle-3) &
             + dens(4*radius,4*angle-2) &
             + dens(4*radius,4*angle-1) &
             + dens(4*radius,4*angle)) *r_prime
     end do
  end do
...
! Best time Time (seconds) =   0.759480000 
