        ...
        do j = 1, N_theta0
            ...
            do iprime = 0, N_rx
                do jprime = 1, N_thetax
                    if (((iprime-iconv)<1 .AND. (iprime-iconv)>-2) .AND. ((jprime-jconv)<1 .AND. (jprime-jconv)>-2)) then ! (ABS(iprime-iconv)<2 .AND. ABS(jprime-jconv)<2) then
                        !print *,"i,     j     ",i,     j
                        !print *,"iprime,jprime",iprime,jprime
                        do iref = max(2*(iprime-1)+1,0), min(2*(iprime-1)+2,N_rxref)
                            do jref = max(2*(jprime-1)+1,1), min(2*(jprime-1)+2,N_thetaxref)
                                vmass = (c1ref * massxref(iref, jref)    +&
                                        &c2ref * massxref(iref+1, jref)  +&
                                        &c3ref * massxref(iref, jref+1)  +&
                                        &c4ref * massxref(iref+1, jref+1))&
                                        & * inv_factor2ref
                                ! calculate shifted values
                                rprime = rxref(iref)+dr_shiftref
                                ratio = r_corner/rprime
                                delta_theta = theta_corner-thetaxref(jref)-dtheta_shiftref
