...
! READ THE LEVEL (FROM THE ARGUMENT LINE)
    ...
    if (LEVEL==1) then
        ...
        epsilon2 = .000500      ! try different values
    else
        epsilon2 = 0.002        ! try different values
    end if
        ...
        do j = 1, N_theta0
            ...
            do iprime = 1, N_rx
                do jprime = 1, N_thetax
                    ...
                    denom_point = sqrt(1+rx_ratio_squared(i, iprime)-2*rx_ratio(i, iprime)*cos_tablex(j, jprime)+epsilon2)
                    ...
                    force_point = massx(iprime,jprime)/(denom_point*denom_point*denom_point*rx_prime_squared(iprime))
