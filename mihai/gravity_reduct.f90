       ...
       do k = 1,nr
         do l = 1, nt
            acc(1) = acc(1) + mass(k,l)* & 
                    (radius(k)/radius_corn(i)*cos_center(l) - & 
                    cos_corner(j)) & 
                    /((radius(k)/radius_corn(i))**2 +1 -2*radius(k) & 
                    /radius_corn(i)*cos_diff(l,j))**(1.5)/radius_corn(i)**2 
            acc(2) = acc(2) + mass(k,l)* &
                    (radius(k)/radius_corn(i)*sin_center(l) - &
                    sin_corner(j)) &
                    /((radius(k)/radius_corn(i))**2 + 1 -2*radius(k) &
                    /radius_corn(i)*cos_diff(l,j))**(1.5)/radius_corn(i)**2
         enddo
...
OPEN(UNIT = 7, FILE='/home/ics/mihai/git/Computational_Science_II_Open/time_reduction.data')
