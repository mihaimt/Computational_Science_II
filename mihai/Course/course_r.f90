...
type FUNCTION course_density(nr, nt,  density)
IMPLICIT NONE

REAL, dimension(1:nr/2, 1:nt/2) :: density_c


do i = 1, nr/2
       
        do j = 1, nt/2
                density_c(i,j) = (density(2*i-1, 2*j-1)+ density(2*i-1, 2*j) + &
                                density(2*i, 2*j-1) + density(2*i, 2*j))/4.
               
        enddo
enddo


course_density = density_c
END FUNCTION course_density
...
density_c = course_density(nr, nt, density)
