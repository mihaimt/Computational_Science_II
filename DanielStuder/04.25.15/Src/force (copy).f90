...
do j=1,dim_theta
	do i=1,dim_r
		...
		do l=1+level_halfsize,dim_theta,level_size
			do k=1+level_halfsize,dim_r,level_size
				!if(mod(i,level_size)>0.and.mod(j,level_size)>0) then
					var0=cos_dtheta(j,l)
					var1=sin_dtheta(j,l)
					var2=r_sub(i,k)
					var3=1+var2*var2-2*var2*var0
					var3=var3*sqrt(var3)
					var3=mass(level,k,l)/(var3+0.000001)/r(k)
					dforce_r=dforce_r+(var2-var0)*var3
					dforce_theta=dforce_theta+var1*var3
				!end if
				!if(mod(i,level_size)==0.and.mod(j,level_size)>0.and.k+level_size<=dim_r) then
				!	var0=cos_dtheta(j,l)
				!	var1=sin_dtheta(j,l)
				!	var2=r_sub(i,k+level_halfsize)
				!	var3=1+var2*var2-2*var2*var0
				!	var3=var3*sqrt(var3)
				!	var3=(mass(level,k,l)+mass(level,k+level_size,l))/2/(var3)/r(k+level_halfsize)
				!	dforce_r=dforce_r+(var2-var0)*var3
				!	dforce_theta=dforce_theta+var1*var3
				!end if
				!if(mod(i,level_size)>0.and.mod(j,level_size)==0.and.l+level_size<=dim_theta) then
				!	var0=cos_dtheta(j,l+level_halfsize)
				!	var1=sin_dtheta(j,l+level_halfsize)
				!	var2=r_sub(i,k)
				!	var3=1+var2*var2-2*var2*var0
				!	var3=var3*sqrt(var3)
				...
				!	dforce_theta=dforce_theta+var1*var3
				!end if
				!if(mod(i,level_size)==0.and.mod(j,level_size)==0.and.k+level_size<=dim_r.and.l+level_size<=dim_theta) then
				!	var0=cos_dtheta(j,l+level_halfsize)
				!	var1=sin_dtheta(j,l+level_halfsize)
				!	var2=r_sub(i,k+level_halfsize)
				!	var3=1+var2*var2-2*var2*var0
				!	var3=var3*sqrt(var3)
				!	var4=mass(level,k,l)/4/(var3)/r(k+level_halfsize)
				!	var4=var4+mass(level,k+level_size,l)/4/(var3)/r(k+level_halfsize)
				!	var4=var4+mass(level,k+level_size,l+level_size)/4/(var3)/r(k+level_halfsize)
				!	var4=var4+mass(level,k,l+level_size)/4/(var3)/r(k+level_halfsize)
