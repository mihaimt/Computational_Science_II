...
! build mass grid level 1_________________________________________________
...
do k=1+halfsize,dim_r,levelsize
	do l=1+halfsize,dim_theta,levelsize
		do j=1,(2**level)
			do i=1,(2**level)
				mass(k,l)=mass(k,l)-G*density((k-1)*(2**level)+i,(l-1)*(2**level)+j)*dtheta*dr/r(k)
			end do
		end do
		!do i=k-halfsize,k+halfsize-1
			!do j=l-halfsize,l+halfsize-1
			!	mass(k,l)=mass(k,l)+density(i,j)*dtheta*dr
			!end do
		!end do
	end do
end do
...
do i=1+halfsize,dim_r,levelsize
	...
	write(*,*) mass(i,1+halfsize)
	
end do
call CPU_Time(t_end)				
t=t_end-t_start
write(*,*) "required time generating mass grid level 1"
write(*,*) t,"seconds"
	...
	do i=1,dim_r
		...
		do l=1+halfsize,dim_theta,levelsize
			...
			do k=1+halfsize,dim_r,levelsize
				...
				var5=var5*sqrt(var5)
				var6=-G*mass(k,l)/(var5+0.0000001)/r(k)
