	...
	do i=1,dim_r
		...
		do l=1+level_halfsize,dim_theta,level_size
			do k=1+level_halfsize,dim_r,level_size
				...
				if(pos_r>0.and.pos_theta==0.and.k+level_size<=dim_r) then
					var0=cos_dtheta(j,l)
					var1=sin_dtheta(j,l)
				...
				if(pos_r==0.and.pos_theta>0.and.l+level_size<=dim_theta) then
				...
				if(pos_r>0.and.pos_theta>0.and.k+level_size<=dim_r.and.l+level_size<=dim_theta) then
					var0=cos_dtheta(j,l+level_halfsize)
					var1=sin_dtheta(j,l+level_halfsize)
					var2=r_sub(i,k+level_halfsize)
					var3=1+var2*var2-2*var2*var0
...
open(unit=4,file="../output/force_project.data")


do j=1,dim_theta
	do i=1,dim_r

		write(4,'(2e20.10)') force(1,i,j)
	end do
end do
close(4)
